...
int main() {
    ...
    while (true) {
        ...
        int temp = walet;
        sem_wait(sem_paper);
        while (msgrcv(msgid_product, &product_paper, sizeof(Product), 2, IPC_NOWAIT) != -1) {
        ...
        if (shm_prices->matches + shm_prices->tabaco <= walet) {
            ...
            printf("\tmatches: %d\n\ttabaco: %d\n", product_matches.price, product_tabaco.price);
        }
        sem_post(sem_prices);

        if (flag) {
            flag = false;
            // printf("\nwalet: %d\n\tproduct_matches: %d\n\tproduct_tabaco: %d\n", walet, product_matches.price, product_tabaco.price);
