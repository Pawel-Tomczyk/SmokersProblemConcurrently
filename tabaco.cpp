...
int main() {
    ...
    while (true) {
        ...
        int temp = walet;
        sem_wait(sem_tabaco);
        while (msgrcv(msgid_product, &product_tabaco, sizeof(Product), 3, IPC_NOWAIT) != -1) {
        ...
        if (shm_prices->matches + shm_prices->paper <= walet) {
            flag = true;
            product_matches.price = shm_prices->matches;
            product_paper.price = shm_prices->paper;
            printf("\tmatches: %d\n\tpaper: %d\n", product_matches.price, product_paper.price);
        }
        sem_post(sem_prices);

        if (flag) {
            flag = false;
            // printf("\nWalet: %d\t product_matches price: %d\t product_paper price: %d\n", walet, product_matches.price, product_paper.price);
            walet -= product_matches.price;
            walet -= product_paper.price;

            sem_wait(sem_matches);
            msgsnd(msgid_product, &product_matches, sizeof(Product), 0);
